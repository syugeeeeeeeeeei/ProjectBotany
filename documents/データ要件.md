...
- `currentTurn: number`
  現在のターン数を示す数値です。
- `maximumTurns: number`
  ゲームが終了する総ターン数を示す数値です（例：8 ターン）。この数に達すると勝敗が決まります。
...
- `cardDefinitionId: string`
  この外来種コマが、元々どの「カードの定義データ」から生まれたのかを示す ID です。
- `currentX: number`
  その外来種コマがフィールドのどのマス（X 座標）に現在置かれているかを示す数値です。
- `currentY: number`
  その外来種コマがフィールドのどのマス（Y 座標）に現在置かれているかを示す数値です。
...
- `usageLimit?: number | null`
  そのカードが 1 ゲーム中に使用できる最大回数です。`null`の場合は無制限です。
