...
export const useGameQuery = {
	...
	/** 特定プレイヤーの状態を取得 */
	usePlayer: (playerId: PlayerType) => useGameStore((state) => state.playerStates[playerId]),
...
export const gameQuery = {
	...
	/** 特定プレイヤーの状態を取得 */
	player: (playerId: PlayerType) => useGameStore.getState().playerStates[playerId],

	/** 盤面データを取得 */
	field: () => useGameStore.getState().gameField,

	/** 特定のセルを取得 */
	cell: (x: number, y: number) => useGameStore.getState().gameField.cells[y]?.[x],
};