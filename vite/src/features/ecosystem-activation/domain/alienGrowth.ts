 ...
 */
export const applyGrowthLogic = (alien: ActiveAlienInstance, cardDef: AlienCard) => {
	if (!cardDef.canGrow || !cardDef.growthConditions || !cardDef.growthEffects) return;
	...
	if (conditionMet) {
		if (nextEffect.newInvasionPower) alien.currentInvasionPower = nextEffect.newInvasionPower;
		if (nextEffect.newInvasionShape) alien.currentInvasionShape = nextEffect.newInvasionShape;
