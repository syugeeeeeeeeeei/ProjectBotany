...
const Hand3D: React.FC<Hand3DProps> = ({
	...
	const handleHorizontalFlick = (state: DragState) => {
		...
		// 条件判定
		const isFlickDistanceMet =
			absMx > GESTURE_SETTINGS.FLICK_DISTANCE_THRESHOLD;
	...
	const handleVerticalFlick = (state: DragState) => {
		...
		// 条件判定
		const isFlickDistanceMet =
			absMy > GESTURE_SETTINGS.FLICK_DISTANCE_THRESHOLD;
...
const CardInLine: React.FC<CardInLineProps> = ({
	...
	const { z, opacity } = useSpring({
		...
		opacity: isListVisible
			...
			isSelected
				? HAND_LAYOUT.CARD_IN_LINE_ANIMATION.OPACITY_VISIBLE
				: HAND_LAYOUT.CARD_IN_LINE_ANIMATION.OPACITY_HIDDEN,
